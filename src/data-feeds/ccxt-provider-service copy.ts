...
export class CcxtFeed implements BaseDataFeed {
  ...
  async start() {
    ...
    void this.watchTickers(exchangeToSymbols);
  }
  ...
  private async watchTickers(exchangeToSymbols: Map<string, Set<string>>) {
    for (const [exchangeName, symbols] of exchangeToSymbols) {
      const exchange = this.exchangeByName.get(exchangeName);
      if (exchange === undefined) {
        this.logger.warn(`Exchange ${exchangeName} skipped`);
      ...
      this.logger.log(`Watching tickers for ${marketIds} on exchange ${exchangeName}`);
      ...
      if (exchange.has["watchTickers"]) {
        ...
        while (true) {
          try {
            ...
            this.logger.error(`Failed to watch tickers for exchange ${exchangeName}: ${e}`);
            return;
          }
        }
        ...
        while (true) {
          try {
            ...
            this.logger.error(`Failed to fetch tickers for exchange ${exchangeName}: ${e}`);
            return;
          }
        }
      } else {
        this.logger.warn(`Watching tickers method not found for exchange ${exchangeName}`);
      }
    }
  }

  private processTickers(tickers: Tickers, exchangeName: string) {
    Object.entries(tickers).forEach(([key, ticker]) => {
      if (!ticker.symbol.endsWith("USDT")) {
        this.logger.warn(`Unsupported quote tickers for exchange ${exchangeName}`);
        return;
      }

      const volumes = this.volumes.get(ticker.symbol) || new Map<string, VolumeInfo>();
      volumes.set(exchangeName, {
        volume: ticker.quoteVolume,
        time: ticker.timestamp,
        exchange: exchangeName,
      });
      this.logger.debug(`volumes ${volumes} when tickers ${tickers} on exchange ${exchangeName}`); // TODO
      this.volumes.set(ticker.symbol, volumes);
    });
  }
  ...
  private async getFeedPrice(feedId: FeedId): Promise<number | undefined> {
    ...
    for (const source of config.sources) {
      ...
      this.logger.debug(`price info ${info}`);
